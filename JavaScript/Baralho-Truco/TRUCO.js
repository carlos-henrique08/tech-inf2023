const Deck_id = "ngifly5ann4s";
const deck = new Deck(Deck_id);
...
function Deck(Deck_id){
 ...
 this.olharCartas = function(){
  fetch(`https://deckofcardsapi.com/api/deck/${Deck_id}/draw/?count=3`)
    ...
    this.NVDeck = function(){
     fetch(`https://deckofcardsapi.com/api/deck/ngifly5ann4s/shuffle/?cards=${this.listOfCards}`)
      .then (getJsonData)
      .then (atualizainfo)
      .then(function(jsonData){
